...
- `/finance/wishlist` 心动清单复用股票与 ETF 收盘快照。14 个初始目标是用户授权展示的参考预设，个人编辑仅保存到浏览器，支持 JSON 合并导入和导出；不将本地修改回写公共快照。严格区分低于与不高于，默认接近范围为目标价上方 3%；超过 4 个自然日、未来日期或缺失行情不触发达标。指数点位暂仅作备注。计算和导入校验见 `apps/web-antd/src/views/finance/wishlist/model.test.ts`。 本地清单使用v3存储：首次读取v2时仅补入海尔智家（600690，≤19.50元）；v1先补入森马服饰（002563，≤5元），再迁移至v3。保留已有编辑和删除，v3后续删除不自动恢复。
...
- 自选与持仓名称旁的爱心读取同一浏览器的心动清单目标价，按距目标需跌 ≤2%、≤5%、≤10%、≤15%、≤20%、≤30%、>30% 七档着色，数值沿用心动清单 evaluate 口径。过期或缺失行情用中性空心爱心；存储读取失败明确提示。手机操作列不固定，避免遮住名称及爱心。

## 14. 研发工具、日期与日历

- `/engineering` 提供 JSON、文本对比、SQL、时间编码、正则、知识库与 Cron。输入默认在浏览器本地处理；SQL 不执行，Cron 不创建定时任务，正则在可终止 Worker 中运行。知识库和正则收藏共用本地存储，导入严格校验。
- `/life-tools/dates` 的节假日数据与来源保存在 `dates/holidays.json`，当前覆盖 2026 年。覆盖年份必须完整维护后才可声明；未知年份不按中国节假日口径给出工作日结果。年度事项仅支持公历，2 月 29 日的平年处理由用户选择。
