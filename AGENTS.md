...
- `/finance/wishlist` 心动清单复用股票与 ETF 收盘快照。12 个初始目标是用户授权展示的参考预设，个人编辑仅保存到浏览器，支持 JSON 合并导入和导出；不将本地修改回写公共快照。严格区分低于与不高于，默认接近范围为目标价上方 3%；超过 4 个自然日、未来日期或缺失行情不触发达标。指数点位暂仅作备注。计算和导入校验见 `apps/web-antd/src/views/finance/wishlist/model.test.ts`。
